...
export function Topology() {
  ...
  async function edit(link: any, reverse = false) {
    if (link.protocol !== "manual") {
      setMessage("Les relations LLDP/CDP et IPAM sont en lecture seule");
      return;
    }
    const source = reverse
      ? link.source_port
      : prompt("Port source", link.source_port || "");
    ...
    try {
      await api("/topology/links/" + link.id, {
        method: "PATCH",
        body: JSON.stringify({
          source_port: source || null,
          target_port: target || null,
          reverse,
        }),
      });
      setMessage(reverse ? "Sens de la relation inversé" : "Ports mis à jour");
      load();
    } catch (x: any) {
      setMessage(x.message);
    }
  ...
  async function remove(link: any) {
    ...
    try {
      await api("/topology/links/" + link.id, { method: "DELETE" });
  ...
  const links = data.links.filter(
    (x: any) => filter === "all" || x.protocol === filter,
  );
  return (
    <Layout title="Topologie réseau">
      <div className="pageHead">
        ...
        >
          ...
          <option value="inferred_ipam">IPAM inférées</option>
        </select>
        {editable && (
            ...
            <div className="topologyLink" key={link.id}>
              ...
              {editable && (
                <div className="relationActions">
                  {link.protocol === "manual" && (
                    <>
                      <button
                        title="Modifier les ports"
                        onClick={() => edit(link)}
                      >
                        <Pencil />
                      </button>
